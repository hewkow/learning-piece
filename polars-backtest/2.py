...
print(
    df.group_by('trade_id').agg(
        pl.col('entry_value').last(),
        pl.col('exit_value').last(),
        pl.col('realize').last()
    ).sort('trade_id')
)
...
fig_eq = go.Figure()
fig_eq.add_trace(go.Scatter(y=df['entry_value'],mode='lines+markers',name='entry value'))
fig_eq.add_trace(go.Scatter(y=df['exit_value'],mode='lines+markers',name='exit value'))
fig_eq.add_trace(go.Scatter(y=df['realize'],mode='lines+markers',name='realize'))
fig_eq.add_trace(go.Scatter(y=df['trade_id'],mode='lines+markers',name='trade id'))
